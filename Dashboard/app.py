...
#path_to_blank = 'BLANK_SKETCH_updated.jpg'
# slider 
orientation_perc = 1
MA_perc = 1
ma_perc = 1
...
app.layout = html.Div(
    [
     ...
     html.Br(),
     dbc.Row([
         dcc.Input(id="pathinput", type="text", placeholder="path/to/images/", style = {'margin-left': '12px', 'width': '40%'}),
         dbc.Button(id="go", children="Go", n_clicks = 0, color="primary", style={"margin-left": "15px"}),
         html.Div(id='hidden-div9', style={"margin-left": "15px"}),                    
         ]),
     html.Br(),
     dbc.Row(
         [
             dbc.Col(
                 dbc.Card(
                    [
                       ...
                       #html.Hr(),
                       dbc.Button(id="clear", children="Update Weights", style = {'width': '76%', 'margin-left': '30px', 'margin-right': '10px'}),
...
my_im = app.get_asset_url('BLANK_SKETCH_updated.jpg')
type(my_im)
...
def on_button_click(n, n2, run):
    ...
    if "right_click" in changed_id:
        if abs(count) == len(name_info):
            count = 0
        count += 1
        return return_image(count), html.H6('Score:    ' + str(score_html), style = {'width':'100%', 'textAlign': 'left'}), html.H5(name_html, style = {'width':'100%', 'textAlign': 'center'}), html.H5('Matches:    ' + str(n_html) + '/' + str(num_matches_html), style = {'textAlign': 'right'})
...
def update_output(input1, input2):
    ...
    try:
        ...
        return dbc.Badge("Import Successful", color="success", style = {'vertical-align': 'middle'})
    except:
        return dbc.Badge("Import Failed", color="danger", style = {'vertical-align': 'middle'})
